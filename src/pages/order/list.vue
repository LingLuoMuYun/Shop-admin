            ...
            <Search :model="searchForm" @search="getData" @reset="resetSearchForm" >
                ...
                <template #show>
                    ...
                    <SearchItem label="开始时间">
                        <el-date-picker v-model="searchForm.starttime" type="date" placeholder="开始日期" style="width: 90%;" value-format="YYY-MM-DD"/>
                    </SearchItem>
                    <SearchItem label="结束时间">
                        <el-date-picker v-model="searchForm.endtime" type="date" placeholder="结束日期" style="width: 90%;" value-format="YYY-MM-DD"/>
                    </SearchItem>
                </template>
            </Search> 
                    ...
                    <template #default="{row}">
                        <div>
                            ...
                            <div class="flex py-2" v-for="(item,index) in row.order_items" :key="index">
                                ...
                                :lazy="true" style="width: 30px;height:30px;"></el-image>
                                <p class="text-blue-500 ml-2">
                                    {{ item.goods_item ? item.goods_item.title : '商品已删除' }}
                                </p>
                            </div>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="实付款" width="120" prop="total_price" align="center" />
